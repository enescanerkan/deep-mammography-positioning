...
class AttentionGate(nn.Module):
    ...
    like pectoral muscle line and nipple position.
...
class CRAUNet(BaseModel):
    """
    Coordinate-aware Residual Attention UNet for MLO landmark detection.
    
    Combines coordinate convolution with attention gates for precise
    landmark localization in MLO mammography images.
    ...
    Args:
        ...
        out_features: Number of output coordinates (6 for MLO: pec1, pec2, nipple)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Predicted coordinates of shape (B, 6):
                - pec1_x, pec1_y: First pectoral line point
                - pec2_x, pec2_y: Second pectoral line point
                - nipple_x, nipple_y: Nipple center position
        """
        # Encoder path
        e1 = self.enc1(x)
...
def create_model(
    ...
    Args:
        ...
        out_features: Number of output coordinates:
            - 6 for 'all': pec1(x,y), pec2(x,y), nipple(x,y)
            - 4 for 'pectoralis': pec1(x,y), pec2(x,y)
            - 2 for 'nipple': nipple(x,y)
        dropout_rate: Dropout rate for regularization
