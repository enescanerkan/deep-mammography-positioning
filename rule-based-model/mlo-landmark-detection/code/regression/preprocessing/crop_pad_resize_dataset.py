...
class MammogramDataset:
    ...
    def process_and_save_images(self, idx):
        ...
        # Try .dicom first, fallback to .dcm for compatibility
        img_path = os.path.join(self.base_image_dir, f"{row['SOPInstanceUID']}.dicom")
        if not os.path.exists(img_path):
            img_path = os.path.join(self.base_image_dir, f"{row['SOPInstanceUID']}.dcm")
...
def main():
    ...
    annotations_file = str(PROJECT_ROOT / 'labels' / 'mlo_labels.csv')
    base_image_dir = str(PROJECT_ROOT / 'data' / 'raw' / 'mlo')           # Raw DICOM files
    base_landmark_dir = str(MLO_ROOT / 'landmark_coords')                  # Landmark JSONs (model-specific)
    output_dir = str(PROJECT_ROOT / 'data' / 'processed' / 'mlo')         # Processed NPY files
    
    print("=" * 60)
    print("MLO PREPROCESSING")
    print("=" * 60)
    ...
    print(f"Images to process: {len(filtered_indices)}")

    for idx in tqdm(filtered_indices, desc="Processing MLO Images"):
        dataset.process_and_save_images(idx)

    dataset.finalize()
    print("[OK] MLO Preprocessing completed!")
